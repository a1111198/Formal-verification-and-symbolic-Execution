        ...
        assembly {
            ...
            if mul(
                ...
                gt(
                    x,
                    div(
                        not(0), // if y = 0 we will get div(not(x),y)=0 else max/y
                        y
                    )
                ) // If x = 0 we will get
            ) {
                //@audit balnk revert
                //@audit Wrong Functional selector
                mstore(0x40, 0xbac65e5b) // `MathMasters__MulWadFailed()`.  It is wrong as selector is "0xa56044f7".
                revert(0x1c, 0x04) ///
            }
            z := div(mul(x, y), WAD)
        }
    ...
    function mulWadUp(uint256 x, uint256 y) internal pure returns (uint256 z) {
        ...
        assembly {
            ...
            if mul(y, gt(x, div(not(0), y))) {
                //@audit balnk revert
                //@audit Wrong Functional selector
                //@audit need to have new Error Declaration
                mstore(0x40, 0xbac65e5b) // `MathMasters__MulWadFailed()`.
                revert(0x1c, 0x04)
            }

            //@audit for y<= x <2*y  we will fall in that condition and possibaly get inCorrect Results.
            // so remove this we don't need this
