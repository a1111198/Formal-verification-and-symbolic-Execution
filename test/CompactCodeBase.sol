...
contract CompactCodeBase {
    ...
    function uniSqrt(uint256 y) external pure returns (uint256) {
        uint256 z;
        if (y > 3) {
            z = y;
            uint256 x = y / 2 + 1;
    ...
    function sqrtUpperHalf(uint256 x) external pure returns (uint256 z) {
    ...
    function solmateUpperHalf(uint256 x) public pure returns (uint256 z) {
        assembly {
            let y := x
