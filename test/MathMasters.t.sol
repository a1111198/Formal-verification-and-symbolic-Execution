...
contract MathMastersTest is Base_Test {
    ...
    // function testMulWadRevert() public pure {
    //     assertEq(MathMasters.mulWad(type(uint256).max, 271), 0);
    // }
    ...
    // function testMulWadUpFuzzNew() public {
    ...
    //     uint256 x = 2021676786187582165612072;
    //     uint256 y = 2021676786187582165612072;
    ...
    function check_testMulWadUpFuzz1(uint256 x, uint256 y) public pure {
        if (x == 0) {
            uint256 result = MathMasters.mulWadUp(x, y);
            uint256 expected = x * y == 0 ? 0 : (x * y - 1) / 1e18 + 1;
            assert(result == expected);
    ...
    function check_testMulWadUpFuzz2(uint256 x, uint256 y) public pure {
        if (y == 0) {
            uint256 result = MathMasters.mulWadUp(x, y);
            uint256 expected = x * y == 0 ? 0 : (x * y - 1) / 1e18 + 1;
            assert(result == expected);
    ...
    function check_testMulWadUpFuzz3(uint64 x, uint64 y) public pure {
        if (y <= type(uint64).max / x) {
            uint256 result = MathMasters.mulWadUp(x, y);
            uint256 expected = x * y == 0 ? 0 : (x * y - 1) / 1e18 + 1;
            assert(result == expected);
        }
    ...
    function testSqrtUniHalf() public pure {
        uint256 x = 0xfffffeffffffffffffffffffffffffffffffffffffffff;
        assert(MathMasters.sqrt(x) == solmateSqrt(x));
    }

    function checkSqrtFunction(uint256 x) public pure {
        assert(MathMasters.sqrt(x) == uniSqrt(x));
    }
}
